# Lab01S01 : Planejamento e prototipação do site

## Objetivos da Sprint
Planejamento inicial, criação de repositório, wireframes e protótipo inicial do front-end.

## Tarefas e Evidências

- [ ] Criação do repositório GitHub com README inicial
  - 📄 **Evidência:** [Link para o Repositório/Print](./evidencias/repositorio_github.pdf)
- [ ] Wireframes das páginas no figma (média fidelidade)
  - 📄 **Evidência:** [Link para o Figma/Print](./evidencias/wireframes_figma.pdf)
- [ ] Protótipo inicial do front-end (HTML/CSS/JS ou frameworks modernos)
  - 📄 **Evidência:** [Link para o código/Print](./evidencias/prototipo_inicial.png)
- [ ] Implementação da navegação e do layout principal (cabeçalho, rodapé e conteúdo)
  - 📄 **Evidência:** [Link para gravação/Print](./evidencias/navegacao_layout.mp4)

## Entrega
- README com imagens dos protótipos, descrição do projeto, tecnologias previstas e estrutura inicial do site.
  - 📄 **Evidência da Entrega:** [Link para README atualizado](./evidencias/readme_sprint1.md)
